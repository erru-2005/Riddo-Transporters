...
export const registerUser = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const cleanNumber = phone_number.trim().replace(/[\s\-().]/g, '');
    const formattedNumber = cleanNumber.startsWith('+') ? cleanNumber : `+${cleanNumber}`;

    if (!/^\+[1-9]\d{9,14}$/.test(formattedNumber)) {
      return res.status(400).json({
        success: false,
        message: "Invalid phone number format. Please include country code (e.g. +919538820068)"
      });
    }
